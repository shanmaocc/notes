...
if [ -z "$GITHUB_TOKEN" ]; then
  ...
  githubUrl=https://shanmaocc:${GITHUB_TOKEN}@github.com/shanmaocc/notes.git
  git config --global user.name "shanmaocc"
  git config --global user.email "shanmaocc@gmail.com"
fi

# 在 dist 目录下初始化一个新的 git 仓库
git init
git add -A
git commit -m "${msg}"
